        ...
        let sketchSwans = function(p) {
            ...
            class Swan {
                constructor(x, y, sizeFactor = 1) {
                    ...
                    this.velocity = p.createVector(p.random(-1, 1), p.random(-1, 1)); // Initial random direction
                    this.speed = p.random(0.5, 1.0) * this.sizeFactor; // Swan's preferred speed
                ...
                draw() {
                    ...
                    // Neck (simplified)
                    p.stroke(255); // White neck
                    p.strokeWeight(this.bodyWidth / 2.5);
                    // Line from mid-body towards where head will be
                    p.line(0, -this.bodyLength / 2.5, 0, -this.bodyLength * 0.6); 
                    p.noStroke(); // reset stroke for head
                    ...
                    // Beak
                    p.fill(255, 165, 0); // Orange beak
                    p.triangle(0, -this.bodyLength * 0.7 - this.bodyWidth * 0.3, // Tip of beak (further out)
                               -this.bodyWidth * 0.25, -this.bodyLength * 0.7, // Base 1
                               this.bodyWidth * 0.25, -this.bodyLength * 0.7); // Base 2
                    p.pop();
                }

                update() {
                    this.velocity.rotate(p.random(-0.05, 0.05)); // Gentle random turning
                    this.velocity.setMag(this.speed); // Maintain consistent speed
            ...
            p.setup = function() {
                ...
                motherSwan = new Swan(p.random(p.width), p.random(p.height), 1.5); // Mother swan is a bit larger, random initial position
            };

            p.draw = function() {
                p.clear(); // Clear background for transparency (assuming CSS will handle actual background)
                // Or use a semi-transparent background for trails: p.background(0,0,0,10);
                motherSwan.update();
                motherSwan.draw();
            };
